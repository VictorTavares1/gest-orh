...
use App\Models\PedidoHorasExtras;
use App\Models\TipoPedido;
use App\Models\Utilizador;
use Illuminate\Support\Facades\DB;

class CriarHorasExtrasAction
{
    public function execute(Utilizador $user, array $dados): Pedido
    {
        return DB::transaction(function () use ($user, $dados) {
            $tipo   = TipoPedido::where('nome', TipoPedidoEnum::HORAS_EXTRAS->label())->firstOrFail();
            ...
            PedidoHorasExtras::create([
                'id_pedido'    => $pedido->id_pedido,
                'data_horas'   => $dados['data_horas'],
                'numero_horas' => $dados['numero_horas'],
                'motivo'       => $dados['motivo'],
            ]);

            return $pedido->load(['tipoPedido', 'estadoPedido', 'utilizador', 'horasExtras']);
        });
    }
}
